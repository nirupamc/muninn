...
export function MemoryGraph() {
  ...
  const rendererGraph = useMemo(
    () => cloneForRenderer(domainNodes, domainEdges, mode === "3d" ? spatialLimit : domainNodes.length),
    [domainNodes, domainEdges, mode, spatialLimit],
  );
  ...
  return <div className="graph-operations relative flex h-full min-h-0 flex-col overflow-hidden">
    ...
    <footer className="graph-footer"><span>VISIBLE <b>{visibleNodeIds.size}</b></span><span>EDGES <b>{visibleLinkIds.size}</b></span><span>SELECTED <b>{selected ? selected.id.slice(0, 8).toUpperCase() : "—"}</b></span>{mode === "2d" && <span>ZOOM <b>{zoom.toFixed(1)}X</b></span>}{contextSelection.result && <button type="button" onClick={contextSelection.clear}>CLEAR CONTEXT TRACE</button>}{!!graph.data?.relationshipFailures && <span className="warning">RELATIONSHIP SOURCES UNAVAILABLE {graph.data.relationshipFailures}</span>}{graph.data && domainEdges.length === 0 && domainNodes.length > 0 && <span className="warning">NO RECORDED RELATIONSHIPS IN THIS SCOPE</span>}</footer>
  </div>;
}
